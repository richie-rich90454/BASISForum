...
app.post('/api/posts', (req, res) => {
  getSessionFromCookie(req, res)
    .then(session => {
      ...
      const id = uuidv4();
      const badgeClass = sanitizedBadge.toLowerCase() === 'question' ? 'badge-question' : 
                        (sanitizedBadge.toLowerCase() === 'tips' ? 'badge-tips' : 
                        (sanitizedBadge.toLowerCase() === 'resource' ? 'badge-resource' : 'badge-discussion'));
      const date = new Date().toISOString();
      const createdAt = new Date().toISOString();
      
      const db = getDatabase();
      db.run('INSERT INTO posts (id, title, content, badge, badgeClass, author, category, date, replies, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
        [id, sanitizedTitle, sanitizedContent, sanitizedBadge, badgeClass, session.email, sanitizedCategory, date, '0 replies', createdAt], 
        function(err) {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Internal server error' });
        ...
        res.json({ id, post: { id, title: sanitizedTitle, content: sanitizedContent, badge: sanitizedBadge, badgeClass, author: session.email, category: sanitizedCategory, date, replies: '0 replies', createdAt } });
      });
    })
    .catch(err => {
      console.error('Database error:', err);
